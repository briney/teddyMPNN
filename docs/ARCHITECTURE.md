...
**mirroring Foundry's module hierarchy and attribute names exactly** so that
`state_dict` keys match. This enables:
...
Fine-tuned checkpoints are saved in Foundry's current format:

```python
{"model": model.state_dict(), "config": config.model_dump(), ...}
```

A `convert_to_legacy` utility is provided for users of the original
...
|---------|-------|-------------|
| `X` | `(B, L, num_atoms, 3)` | Backbone atom coordinates (N, CA, C, O) |
| `X_m` | `(B, L, num_atoms)` | Atom validity mask |
...
- Partner chain side-chain atoms (via side-chain atomization)
- Glycans, metals, cofactors, and other non-protein atoms present in the structure

### Collation

Variable-length structures are padded to the longest in each batch using a
**token-budget-aware collator**: instead of fixed batch sizes, we pack
structures until the total residue count reaches the token budget (default:
10,000 for ProteinMPNN, 6,000 for LigandMPNN). This handles the wide length
...
factor, and K=21 is the vocabulary size.
...
The `model` key is directly loadable by Foundry.
...
│   ├── collator.py                 # TokenBudgetCollator
...
3. Implement weight I/O (`weights/io.py`, `weights/legacy.py`). Verify
   round-trip: load Foundry checkpoint → our model → save → reload in Foundry.
...
7. Implement unified `PPIDataset` and `TokenBudgetCollator`.
