...
**PositionalEncodings:**
- Computes relative residue offsets, clips to ±32, one-hots (66 classes: 65
  offset bins + 1 inter-chain bucket), projects through
  `embed_positional_features` linear
  layer to 16-dim
- `num_positional_features = 2 * max_relative_feature + 1 + 1 = 66`
- Inter-chain pairs get index 65 (the last class)
...
2. Use backbone-only `X` tensor directly: [B, L, 4, 3]
...
Upstream parsed structures retain the full 37-atom representation as
`xyz_37/xyz_37_m`; model batches derive backbone-only `X/X_m` from those
parsed tensors before graph featurization.
...
2. If atomize_side_chains: gather side-chain atoms from `xyz_37/xyz_37_m` as
   additional context, merge with ligand atoms, re-select top 25 by distance
...
- Uses teacher forcing and can score either all residues or a caller-provided
  subset mask, depending on the evaluation task
- Returns log_probs gathered at ground-truth token indices

**Training-view contract:**
- Each training example is one partner-design view, not a full-complex
  all-residue redesign target
- `designed_residue_mask` marks exactly one partner group
- `fixed_residue_mask` marks the complementary conditioning partner
...
- For PPI fine-tuning, partner-chain side-chain atoms added to `Y/Y_m/Y_t` come
  only from residues in `fixed_residue_mask`
...
### 1.6 — Weight I/O and checkpoint conversion
...
**io.py:**
- `load_checkpoint_bundle(path, model, strict=True)` → loads the canonical
  teddyMPNN-native checkpoint bundle
- `save_checkpoint_bundle(path, model, optimizer, scheduler, step, config, metrics, compatibility)` →
  saves the canonical native bundle
...
**foundry.py:**
- `load_foundry_checkpoint(path, model, strict=True)` → loads Foundry-current
  checkpoints directly into our compatible model
- `export_foundry_checkpoint(path, model, config)` → writes a strict-loadable
  Foundry-current checkpoint from a native teddyMPNN bundle/model pair
...
- Round-trip: load legacy → save native bundle → reload native bundle →
  identical state_dict
- Native bundle → Foundry-current export → strict reload in Foundry-compatible
  format
...
2. Extract per-residue 37-atom coordinates → `xyz_37` [L, 37, 3]
3. Extract atom occupancy masks → `xyz_37_m` [L, 37] (bool, True if atom resolved)
...
8. Preserve chain/residue metadata needed to assign partner groups from the
   manifest during dataset construction
...
**identify_interface_residues(xyz_37, chain_labels, distance_cutoff=8.0) → mask:**
...
### 2.5 — Unified dataset and partner-design views
...
- Each two-partner structure expands into two default partner-design views:
  design A conditioned on B, and design B conditioned on A
- `__getitem__` returns a feature dict:
  ```python
  {
      "xyz_37": Tensor,      # (L, 37, 3)
      "xyz_37_m": Tensor,    # (L, 37)
      "X": Tensor,           # (L, 4, 3)
      "X_m": Tensor,         # (L, 4)
      "S": Tensor,           # (L,)
      "R_idx": Tensor,       # (L,)
      ...
      "designed_residue_mask": Tensor,  # (L,) — one partner group only
      "fixed_residue_mask": Tensor,     # (L,) — complementary partner context
...
- For LigandMPNN views, `Y/Y_m/Y_t` include real non-protein atoms plus
  side-chain atoms from residues in `fixed_residue_mask`
...
### 2.6 — Token-budget batch sampler and padding collator
...
- Groups dataset indices until cumulative residue count reaches budget

**PaddingCollator:**
- Pads all tensors in the batch to the longest sequence
- Padding values: xyz_37=0.0, xyz_37_m=False, X=0.0, X_m=False, S=UNK_IDX,
  R_idx=-100, chain_labels=-1, residue_mask=False,
  designed_residue_mask=False, fixed_residue_mask=False, Y=0.0, Y_m=0, Y_t=0

**Integration with DataLoader:**
- `batch_sampler=TokenBudgetBatchSampler(...)`
- `collate_fn=PaddingCollator(...)`
- The collator only pads/stacks the examples chosen by the sampler; it does not
  fetch or accumulate additional dataset items on its own
...
- Forward:
  ...
  4. Mask by `mask_for_loss`, sum numerator and denominator separately
  5. Divide by `max(mask_for_loss.sum(), 1)`

In DDP mode, reduce the masked numerator and denominator across workers before
dividing so the effective loss is still the mean over designed positions.

**Tests:**
- Perfect predictions (log_prob=0 at correct token) → near-zero smoothed loss
- Uniform predictions → loss ≈ log(21)
...
for step in range(max_steps):
    ...
    with autocast(dtype=bf16):
        ...
        loss_dict = loss_fn({"input_features": batch}, output, {})
        loss = loss_dict["loss"]
...
Checkpoint resume:
- Load model, optimizer, scheduler, step from the native checkpoint bundle
- Resume from step+1
